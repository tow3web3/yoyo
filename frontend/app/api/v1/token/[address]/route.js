...
export async function GET(request, { params }) {
  try {
    ...
    return apiJson({
      ...
      split: { holders: Number(config.split_holders_bps ?? 10000), creator: Number(config.split_creator_bps ?? 0), burn: Number(config.split_burn_bps ?? 0), treasury: Number(config.split_treasury_bps ?? 0) },

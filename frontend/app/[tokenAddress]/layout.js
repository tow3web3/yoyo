...
export async function generateMetadata({ params }) {
  ...
  const fallback = { title: 'Token dashboard · Boomerang', description: 'Live dividend dashboard for a token paying its holders in Robinhood Stock Tokens.' };
  ...
  try {
    ...
    const title = `${sym} pays dividends in ${meta[tgt]?.symbol || 'stocks'}${apy ? ` · ${apy} APY` : ''} · Boomerang`;
    const description = `${data.stats.execution_count || 0} dividends paid to holders on Robinhood Chain${apy ? `, ${apy} annualized yield` : ''}. Live dashboard by Boomerang.`;

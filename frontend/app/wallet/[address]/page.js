...
export async function generateMetadata({ params }) {
  ...
  if (!EVM_ADDR.test(address)) return { title: 'Wallet · Boomerang' };
  const image = `${siteUrl()}/api/card/wallet/${address}`;
  const title = `Dividend statement for ${short(address)}`;
  return {
    title: `${title} · Boomerang`,
    description: 'Stock dividends earned by holding tokens on Robinhood Chain, via Boomerang.',
    ...
    <>
      ...
      <main className="mx-auto max-w-4xl px-5 py-10">
        ...
        <div className="panel-ink relative mb-6 overflow-hidden p-6">
          ...
          {dividends === 0 ? (
            <p className="mt-2 text-white/80">No dividend yet for this wallet. Hold a token that runs Boomerang and the statement fills itself.</p>
          ) : (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="panel p-6">
            <h2 className="mb-3 text-sm font-semibold text-ink">Boomerang tokens held</h2>
            {holdings.length === 0 ? <p className="text-sm text-mut">This wallet holds no token that runs Boomerang right now.</p> : (

...
export async function POST(request) {
  try {
    ...
    if (!BOOMERANG_TOKEN) return Response.json({ error: 'Missions open once $YOYO is live' }, { status: 503 });
    ...
    if (balance < MIN_HOLD) {
      return Response.json({ error: `You need at least ${MIN_HOLD.toLocaleString()} $YOYO to earn rewards (you hold ${Math.floor(balance).toLocaleString()}).` }, { status: 403 });
    }
    ...
    if (mission.type === 'hold') {
      ...
      ok = balance >= min;
      reason = `Hold ${min.toLocaleString()} $YOYO (you have ${Math.floor(balance).toLocaleString()}).`;
